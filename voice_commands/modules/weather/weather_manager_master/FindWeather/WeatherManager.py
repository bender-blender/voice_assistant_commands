...
class WeatherManager: # Provider
    ...
    def __init__(self, api_key: str, place_code: str, days_count: int = 7):
    ...
    def get_current(self) -> Current:
        ...
        code = condition_dict.get('code', 1000)  # по умолчанию – солнце

        # Преобразуем данные в объект Current
        new_current = Current(
            cloud=current.get('cloud', 0.0),
            humidity=current.get('humidity', 0.0),
            temp_c=current.get('temp_c', 0.0),
            feels_like_c=current.get('feelslike_c', 0.0),  # исправлено название поля
    ...
    def get_days(self) -> List[Day]:
        ...
        for day_data in forecast_days:
            ...
            day = Day(
                ...
                hours=[]  # можешь отдельно заполнить часовой прогноз
            )
            days.append(day)
